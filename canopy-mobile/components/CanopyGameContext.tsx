...
export type GameMode = "human-vs-human" | "human-vs-monster" | "tutorial";
...
function countLeavesWithUnblockedSun(
  ...
  total += 2; // starter row: 2 circles per player with nothing above
  return total;
}
...
export function CanopyGameProvider({ children }: { children: React.ReactNode }) {
  ...
  const placeLeaf = useCallback(
    (row: number, col: number) => {
      ...
      if (mode === "tutorial") {
        // In tutorial mode, we manually manage sun points and ignore auto-advance-on-zero.
        if (currentPlayer === 1) {
          setPlayer1SunPoints((p) => p - cost);
        } else {
    ...
    [currentPlayer, canPlace, getPlaceCost, gameStarted, isGameOver]
  );
  ...
  const runTutorialOpponentDiagonal = useCallback(() => {
    // Pick a simple diagonal move for player2 from any existing leaf.
    const boardSnapshot = boardRef.current;
    ...
    outer: for (let r = 0; r < ROWS; r++) {
      for (let c = 0; c < COLS; c++) {
        if (boardSnapshot[r][c] === "player2") {
          ...
          if (r > 0 && c > 0 && boardSnapshot[r - 1][c - 1] === null) {
            candidates.push({ row: r - 1, col: c - 1 });
          }
          if (
            r > 0 &&
            c < COLS - 1 &&
            boardSnapshot[r - 1][c + 1] === null
          ) {
            candidates.push({ row: r - 1, col: c + 1 });
          }
    ...
    if (!target) {
      // No available diagonal; end tutorial gracefully.
    ...
    const applyPlacement = (): void => {
      setBoard((prev) =>
        prev.map((r, ri) =>
          ri === target!.row
            ? r.map((c, ci) => (ci === target!.col ? "player2" : c))
            : r
        )
      );
  ...
  const runTutorialOpponentRound3OrLater = useCallback(
    (sunlightStep: TutorialStep, availableSunPoints: number) => {
      ...
      const toApply: MoveWithParent[] = [];
      for (const { m } of scored) {
        if (m.cost <= pointsLeft) {
          toApply.push(m);
          pointsLeft -= m.cost;
        }
      }
      ...
      const applyPlacement = (index: number): void => {
        if (index >= toApply.length) {
          setTutorialStep(sunlightStep);
          setSunlightAnimationActive(true);
          return;
        }

        const p = toApply[index];
        setBoard((prev) =>
          prev.map((r, ri) =>
            ri === p.row
              ? r.map((c, ci) => (ci === p.col ? "player2" : c))
  ...
  const onSunlightAnimationComplete = useCallback(() => {
    ...
    if (mode === "tutorial") {
      ...
      if (
        ...
        const nextR = tutorialStep === "round3-sunlight" ? 4 : round + 1;
        setRound(nextR);
  ...
  return (
    <CanopyGameContext.Provider value={value}>
      ...
      {popup.open && (
        <div
          className={`fixed inset-0 z-[80] flex items-center justifycenter p-4 transition-colors duration-700 ${
